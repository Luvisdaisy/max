...
3. 鼠标坐标只用你看到的最近一帧截图上的视图像素，原点在图左上角，且必须落在该帧 view_width×view_height 内；或使用 ocr_locate 返回的 target_id。不要用逻辑分辨率、屏幕百分比或 0-1000 归一化坐标，也不要把工具摘要或 screen_info 里的逻辑坐标再当输入。
4. 看不清字或需要可点文字框时再调用 ocr 或 ocr_locate；定位之后仍然要用键鼠操作，不能只报文字。
