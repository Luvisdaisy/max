...
3. 点图标或按钮时，先使用最近截图中看到的视图像素调用 mouse_move；不要使用定位编号或猜测旧坐标。mouse_move 后必须看后置截图，确认红十字在目标上才调用无坐标 mouse_click；新截图会使此前的坐标判断失效。鼠标坐标原点在图左上角，且必须落在该帧 view_width×view_height 内。不要用逻辑分辨率、屏幕百分比或 0-1000 归一化坐标，也不要把工具摘要或 screen_info 里的逻辑坐标再当输入。
4. 需要抄录或看不清字时再调用 ocr。识别文字后仍然要用键鼠操作，不能只报文字。
