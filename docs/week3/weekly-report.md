...
项目大纲中第三周的预期交付物为“数据集预处理脚本 + 基础 Agent 框架代码”。实际进展如下：

| 大纲第三周目标 | 完成状态 | 实际成果 |
| --- | --- | --- |
| 调研、下载并预处理公开 GUI 数据集 | 已完成调研与处理方案 | 完成 ScreenAgent、WebArena 的数据形态、用途及统一 JSONL 格式设计；ScreenAgent 作为训练轨迹主数据，WebArena 作为任务卡，Mind2Web 的适用范围已完成调研。 |
| 搭建基础多模态 Agent 框架 | 已完成 | 已落地 LangGraph ReAct 循环，覆盖 Think → Act → Observe；具备中断、最大迭代限制、会话恢复与轻量子任务规划。 |
...
- 保留整图 OCR 用于文字读取；新增基于 OmniParser 的 `locate`，为 UI 元素绘制编号框，模型可通过 `target_id` 选择控件。
...
本周没有把不同性质的数据集混为同一种训练数据，而是先完成数据用途和中间格式的对齐：

| 数据来源 | 在项目中的定位 | 本周处理结论 |
| --- | --- | --- |
| ScreenAgent | 训练与规划示范的主数据 | 可将截图、总任务、子任务、人工修正动作转换为统一 JSONL，并映射到现有 `mouse_click`、`mouse_drag`、`keyboard_type` 等工具协议。 |
| WebArena | 任务卡与功能性评测参考 | 解析任务意图、站点和评估规则；不在本周搭建完整网站环境，也不将其 DOM 动作空间混入桌面坐标工具。 |
| Mind2Web | 网页轨迹补充数据 | 已完成其数据形态、适用范围与接入成本的调研。 |

围绕模型能力提升，仓库已新增 LoRA 微调计划及 OpenSpec 变更规格，明确了训练数据、PEFT 参数、训练流程和评估指标。
